...
Assuming your work repositories are inside the `~/WORK` folder.

1. First create a separate Git config:

```sh
git config -f ~/WORK/.gitconfig user.email "me@yycc.me"
git config -f ~/WORK/.gitconfig user.name "leon"
```
...
```sh
cd ~/repo
git config user.email "silenceace@gmail.com"
git config user.name "Potato"
```
